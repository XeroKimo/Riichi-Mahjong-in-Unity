...
public static class HandPatterns
{
    ...
    //Double Sequence
    static public int Iipeikou(List<Meld> melds, Tile.Face prevalentWind, Tile.Face seatWind, ref MahjongFlags flags)
    {
        List<Meld> sequences = MeldHelpers.GetMeldsByType(melds, Meld.Type.Sequence);
        if(sequences.Count < 2)
            return 0;
        if((flags & MahjongFlags.HP_Ryanpeikou) == MahjongFlags.HP_Ryanpeikou)
            return 0;
        ...
        if(iipeikouCount == 2)
        {
            flags &= MahjongFlags.HP_Ryanpeikou;
            return 0;
        }
        if(iipeikouCount == 1)
            return 1;
    ...
    static public int Ryanpeikou(List<Meld> melds, Tile.Face prevalentWind, Tile.Face seatWind, ref MahjongFlags flags)
    {
        List<Meld> sequences = MeldHelpers.GetMeldsByType(melds, Meld.Type.Sequence);
        if(sequences.Count < 4)
            return 0;
    ...
    static public int Ikkitsuukan(List<Meld> melds, Tile.Face prevalentWind, Tile.Face seatWind, MahjongFlags flags)
    {
        ...
        for(int i = 1; i < Tile.suitCount; i++)
        {
            ...
            for(int j = 0; j < 2; j++)
            {
                ...
                {
                    int multiplier = j * 3;
                    if(IsSequenceValue(sequence, (Tile.Face)(j + multiplier), (Tile.Face)(j + 1 + multiplier), (Tile.Face)(j + 2 + multiplier)))
                    {
                        count++;
    ...
    static public int SanshoukuDoujin(List<Meld> melds, Tile.Face prevalentWind, Tile.Face seatWind, MahjongFlags flags)
    {
        ...
        Tile.Suit baseCompareSuit = Tile.Suit.Honor;
        for(int i = 1; i < Tile.suitCount; i++)
        {
            Tile.Suit suit = (Tile.Suit)i;
            if(meldBySuit[suit].Count > highestSequenceCount)
            {
                baseCompareSuit = suit;
                highestSequenceCount = meldBySuit[suit].Count;
            }
        }

        //For each Sequence
        for(int i = 0; i < meldBySuit[baseCompareSuit].Count; i++)
        {
            int sameCount = 0;
            //For each suit
            for(int j = 0; j < 2; j++)
            {
                Tile.Suit suitToCompare = (Tile.Suit)((((int)baseCompareSuit + j) % 4) + 1);
                if(meldBySuit[baseCompareSuit][i].EqualValues(meldBySuit[suitToCompare][0]))
                    sameCount++;
            }
    ...
    static public bool IsSequenceValue(List<Tile> sequence, Tile.Face first, Tile.Face second, Tile.Face third)
    {
        return sequence[0].face == first && sequence[1].face == second && sequence[2].face == third;
    }
    ...
    static public int SanshouDoukou(List<Meld> melds, Tile.Face prevalentWind, Tile.Face seatWind, MahjongFlags flags)
    {
        ...
        {
            if(baseTile.face == triples[i].tiles[0].face)
                count++;
        }
