...
public class TileHelpers
{
    ...
    public static Dictionary<int, List<Tile>> ArrangeTilesByValue(List<Tile> hand)
    {
        Dictionary<int, List<Tile>> output = new Dictionary<int, List<Tile>>();
        foreach (var tile in hand)
        {
            if (!output.ContainsKey(tile.rawValue))
                output[tile.rawValue] = new List<Tile>();
            output[tile.rawValue].Add(tile);
        }
    ...
    public static List<Tile> SortTiles(List<Tile> tiles, Dictionary<Tile.Suit, int> suitOrder)
    {
        ...
        {
            Debug.LogError("Count is not used as suit for sorting");
            return null;
        }
    ...
    public static List<Tile> SortSequence(List<Tile> sequence)
    {
        ...
        sortedSequnce.Sort((Tile lh, Tile rh) => { return lh.rawValue.CompareTo(rh.rawValue); });
    ...
    static bool MakeAllMelds(List<Tile> tiles)
    {
        ...
        Dictionary<int, List<Tile>> tileByValues = ArrangeTilesByValue(tiles);
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            if (count == 1)
            {
                int[] keys = new int[tileByValues.Count];
    ...
    static List<Tile> FindWaitingTiles(List<Tile> tiles)
    {
        ...
        {
            for (int i = Tile.numberMin; i < Tile.numberMax; i++)
            {
        ...
        {
            Dictionary<int, List<Tile>> tileMap = ArrangeTilesByValue(tiles);
    ...
    static bool FindSequenceLower(Dictionary<int, List<Tile>> tileByValues, int startingValue)
    {
        if (tileByValues[startingValue][0].suit == Tile.Suit.Honor)
            return false;

        int count = 0;
        for (int i = startingValue - 3; i < startingValue + 3; i++, count++)
        {
            if (!tileByValues.ContainsKey(i))
            {
                count = 0;
            }
            if (count == 3)
            {
                tileByValues[i - 2].RemoveAt(0);
                tileByValues[i - 1].RemoveAt(0);
                tileByValues[i].RemoveAt(0);

                if (tileByValues[i - 2].Count == 0)
                {
                    tileByValues.Remove(i - 2);
                }
                if (tileByValues[i - 1].Count == 0)
                {
                    tileByValues.Remove(i - 1);
                }
                if (tileByValues[i].Count == 0)
                {
                    tileByValues.Remove(i);
                }
                return true;
            }
        }
    ...
    static bool FindSequenceHigher(Dictionary<int, List<Tile>> tileByValues, int startingValue)
    {
        if (tileByValues[startingValue][0].suit == Tile.Suit.Honor)
            return false;
    ...
    static bool FindSame(Dictionary<int, List<Tile>> tileByValues, int startingValue)
    {
    ...
    {
        return tile.rawValue == 0 || tile.rawValue == 8;
    }
    ...
    public static bool IsDragon(Tile tile)
    {
        ...
        return tile.face == Tile.Face.Red || tile.face == Tile.Face.White || tile.face == Tile.Face.Green;
    }
